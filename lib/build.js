const exec = require('child_process').exec
const fs = require('fs')
const path = require('path')

module.exports = () => {
    const ngTwFile = path.resolve(process.cwd(), 'ng-tailwind.js')
    if (fs.existsSync(ngTwFile)) {
        const config = require(ngTwFile)
        exec(
            `${path.resolve('./node_modules/.bin/node-sass')} ${config.sourceCSS} ${config.tempCSS}`,
            err => err ? console.error(err) : console.info('Successful Build!')
        )
        
        exec(
            `${path.resolve('./node_modules/.bin/tailwind')} build  ${config.tempCSS} -c ${config.configJS} -o ${config.outputCSS}`,
            err => err ? console.error(err) : console.info('Successful Build!')
        )
    } else {
        console.error('No ng-tailwind.js file exists.\nPlease run `ngx-tailwindcss configure`.\n\nRun `ngx-tailwindcss --help` for assistance,\nor view the Readme at\nhttps://github.com/tehpsalmist/ngx-tailwindcss')
    }
    
    function timeout() {
        
    }
}
